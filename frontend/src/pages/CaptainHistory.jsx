...
function CaptainHistory() {
  ...
  const handleServeOrder = async (orderId) => {
    ...
    try {
      ...
      setSuccess('Order marked as served.');
      loadOrders(true);
    } catch (err) {
      setError(err.message || 'Failed to serve order');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col min-h-0 space-y-4">
      ...
      {loading ? (
        <div className="text-center py-20 text-slate font-semibold text-xs animate-pulse">Loading orders...</div>
      ) : (
